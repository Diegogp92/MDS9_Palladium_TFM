# Insctrucciones para consola cmd:
    ...
    # streamlit run OptiStAI_deploy.py
...
logo_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="Logo_Palladium.jpg",
    repo_type="model"
)
...
# Descargar y cargar dataset auxiliar
df_dropdown_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="Datos_a_extraer.csv",
    repo_type="model"
)

df_dropdown = pd.read_csv(df_dropdown_path, sep = ';')
...
# CONTINENTE
df_pais_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="paises_continentes.csv",
    repo_type="model"
)
df_pais = pd.read_csv(df_pais_path, sep = ';')
...
Metamodelo_LGBM_Precios_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="Metamodelo_LGBM_Precios.pkl",
    repo_type="model"
)

with open(Metamodelo_LGBM_Precios_path, "rb") as f:
...
modelo_lgbm_Grande_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="modelo_lgbm_Grande.pkl",
    repo_type="model"
)
with open(modelo_lgbm_Grande_path, "rb") as f:
    modelo_lgbm_Grande = pickle.load(f)


modelo_lgbm_Mediano_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="modelo_lgbm_Mediano.pkl",
    repo_type="model"
)
with open(modelo_lgbm_Mediano_path, "rb") as f:
    modelo_lgbm_Mediano = pickle.load(f)


modelo_lgbm_Pequeno_path = hf_hub_download(
    repo_id="Diegogp92/TFM",
    filename="modelo_lgbm_Pequeno.pkl",
    repo_type="model"
)
with open(modelo_lgbm_Pequeno_path, "rb") as f:
